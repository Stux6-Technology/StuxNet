...
#define __thiscall __cdecl // test complite C mode
#define __patch_null
#define __Exception
#define __swap_boner
#define _i2p, __i2p, __i2p_mma, __i2p_mma // I2P Protocol

void
subm()
{
    signed int __cdecl sub_10023F0(int a1, int a2, int a3);
} 

void mist(__syscall__)
{
   ...
   _UNKNOWN unk_NULL(_NULL_POINTER Exception); 
   extern struct _NULL_POINTER __Exception;
   panic(%f %i %m + "" + __null_unspecified *__CONSTANT_CFSTRINGS__, unsigned int a5); // killer

   return (*--n, *__patch_null);

}

void migrate(sub_100163F0, int z1, int e1, sub_100164Z3, unsigned __int16 word_100164982)
{
    SystemTimeAsFileTime.Dword = 0;
    for (i <0; i += 1; i++) {

        /* 10017490: using guessed type int dword_10017490; */
        /* 10017494: usign guessed type int dword_10017494; */

        unsigned type char __user_call.v1  [~PerfomanceCount = HighPart];
        call(__user_call.inp<eax> *__sys_call, int addr1);
    }
     
    v1 = ReactorSystem.LowPart ^ ReactorSystem.MedPart ^ ReactorSystem.HighPart * v2;
    
    if ( GetTestInternetController != True ) 
    {
        migrate.v1 const type __int16 __super_call();
        __super_call(_MIGRATETOREACTOR.v3 = ~v3) *sub_10017409E0;
        call(__SDA__)<esi>;
    } // This if/else statement defines how the system will respond when there is an 
      // internet connection and when there isn’t
    else
    {
        v2 = ~(_i2p_connect, sub_10012609F0 * 0xFFFF91AA; i2p_migrate, unsigned __int64 _i2p_migrate);
        net(i2p_connect + "[*] " %f %n %a, __doom_i2p *i2p_MM);
        sub_10016464I0, signed uint con_m[64]; /* 64µM */
        goto v2;
    }

    unsigned char __i2p_mma[64]; /* 64µM's %5 */
